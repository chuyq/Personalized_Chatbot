...
def test(ckpt, emotype, save_dir=" "):
    ...
    for ind in tqdm(range(len(all_wavs2))):
        for img_path in tqdm(allimg_cropped):
            ...
            # kwargs = {
            #     'codec': 'libx264',  # 指定使用 H.264 编解码器
            #     'pix_fmt': 'yuv420p',  # 常用于MP4的像素格式
            #     'quality': 8  # 质量设置，范围通常在0-10，10为最高质量
            #     }
            # # # imageio.mimsave(video_path, predictions_gen, fps=25.0)
            # imageio.mimsave(video_path, predictions_gen,fps=25,**kwargs)
            imageio.mimsave(video_path, predictions_gen, format= "MP4", fps=25.0, codec='libx264')
...
if __name__ == '__main__':
    ...
    argparser.add_argument("--root_wav", type=str, default='./demo/video_processed/W015_neu_1_002', help="emotion type ('ang',  'con',  'dis',  'fea',  'hap',  'neu',  'sad',  'sur')")
    # argparser.add_argument("--root_wav", type=str, default='./demo/video_processed/')
